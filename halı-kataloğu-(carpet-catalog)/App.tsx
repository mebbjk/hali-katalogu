...
// FIX: Combined context creation, provider, and hooks into single files to resolve
// a persistent module loading error ("Cannot read properties of null"). This robust
// pattern eliminates potential circular dependencies.
...
const ModalComponent: React.FC<{ children: React.ReactNode; onClose: () => void; title: string }> = ({ children, onClose, title }) => (
    <motion.div
...
const Button: React.FC<{ onClick?: () => void; children: React.ReactNode; className?: string; type?: 'button' | 'submit' | 'reset'; disabled?: boolean; }> = 
...
function App() {
  ...
  const [selectedCarpet, setSelectedCarpet] = React.useState<Carpet | null>(null);
  const [searchQuery, setSearchQuery] = React.useState('');
  const [matchResult, setMatchResult] = React.useState<Carpet | 'not-found' | null>(null);
  const [isMatching, setIsMatching] = React.useState(false);

  // File input refs for import/export
  const importFileRef = React.useRef<HTMLInputElement>(null);

  const filteredCarpets = React.useMemo(() => {
  ...
  const openModal = React.useCallback((type: Modal, carpet?: Carpet) => {
  ...
  const handleFindMatch = React.useCallback(async (file: File) => {
    setIsMatching(true);
    try {
...
const CarpetDetailModal: React.FC<{ carpet: Carpet; onClose: () => void; onDelete: () => void; }> = ({ carpet, onClose, onDelete }) => {
    ...
    const [activeTab, setActiveTab] = React.useState<'details' | 'edit'>('details');
...
const CarpetFormFields: React.FC<{
    ...
    const { t } = useSettings();
    const { addToast } = useToast();
    const [formData, setFormData] = React.useState<Partial<Carpet>>(carpet || {});
    const [imageFile, setImageFile] = React.useState<File | null>(null);
    const [imagePreview, setImagePreview] = React.useState<string | null>(carpet?.imageUrl || null);
