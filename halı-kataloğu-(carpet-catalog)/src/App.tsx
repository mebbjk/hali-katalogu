...
const CarpetFormModal: React.FC<{ carpet: Carpet | null; onClose: () => void }> = ({ carpet, onClose }) => {
    ...
    const handleAiFill = async () => {
        ...
        setIsAnalyzing(true);
        try {
            const details: Partial<Carpet> = await getDetailsFromImage(imageFile);
            const sanitizedDetails: Partial<Carpet> = {};
            for (const key in details) {
              if (Object.prototype.hasOwnProperty.call(details, key)) {
                const value = details[key as keyof Partial<Carpet>];
                if(typeof value !== 'object' || Array.isArray(value)) {
                    // FIX: Use a type assertion to 'any' to resolve a TypeScript error.
                    // This is due to a known limitation in TypeScript where it cannot correlate
                    // the type of a key with the type of its value in a dynamic property assignment.
                    (sanitizedDetails as any)[key] = value;
                }
              }
            }
    ...
    const addCustomValue = (field: 'size' | 'yarnType', value: string, resetter: React.Dispatch<React.SetStateAction<string>>) => {
                ...
                <div className="p-4 space-y-4 max-h-[65vh] overflow-y-auto">
                    <div className="relative">
                        <div onClick={() => setShowPhotoOptions(true)} className="cursor-pointer w-full h-48 bg-zinc-200 dark:bg-zinc-700 rounded-md flex items-center justify-center mb-2 overflow-hidden">
                            {previewUrl ? <img src={previewUrl} alt="preview" className="w-full h-full object-cover"/> : <span className="text-zinc-500 flex flex-col items-center gap-2"><PhotoIcon className="w-8 h-8"/><span>{t('add_photo')}</span></span>}
                        </div>
                        <AnimatePresence>
                        {showPhotoOptions && (
                    ...
                    {(previewUrl) && (
                        <button type="button" onClick={handleAiFill} disabled={isAnalyzing || !imageFile} className="p-2 w-full flex justify-center items-center gap-2 rounded-md bg-purple-600 text-white hover:bg-purple-700 disabled:bg-purple-400 disabled:cursor-not-allowed">
                    ...
                    <FormField label={t('fixed_sizes')}>
                     ...
                     <FormField label={t('custom_size')}>
                         <div className="flex gap-2">
                            <FormInput name="custom_size" value={customSize} customOnChange={(e) => setCustomSize(e.target.value)} />
                            <button type="button" onClick={() => addCustomValue('size', customSize, setCustomSize)} className="px-4 rounded-md bg-zinc-200 dark:bg-zinc-600 text-sm">{t('add')}</button>
                        </div>
                    </FormField>
                    ...
                    <FormField label={t('custom_yarn_type')}>
                         <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => addCustomValue('yarnType', customYarnType, setCustomYarnType)} className="px-4 rounded-md bg-zinc-200 dark:bg-zinc-600 text-sm">{t('add')}</button>
                        </div>
                    </FormField>
...
const BottomNavBar: React.FC<{ 
    ...
    const settingsIndex = tabs.length + 1; // Position for the settings button indicator
    const indicatorIndex = activeTab === 'settings' ? settingsIndex : activeIndex;
    
    const tabWidth = 68; // Approximate width of a tab item in pixels
    const addOffset = 20; // Extra offset for the add button
            ...
            <div className="flex justify-around items-center p-1 relative">
                ...
                {indicatorIndex !== -1 && (
                    <motion.div
                        ...
                        initial={false}
                        animate={{ x: indicatorIndex > 2 ? (indicatorIndex - 1) * tabWidth + addOffset : indicatorIndex * tabWidth }}
                 ...
                 {tabs.map((tab) => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex flex-col items-center justify-center gap-1 p-2 rounded-full transition-colors w-16 h-16 z-10 ${activeTab === tab.id ? 'text-purple-600 dark:text-purple-400' : 'text-zinc-500 hover:text-zinc-800 dark:hover:text-white'}`}>
                        <tab.icon className="w-6 h-6" />
                        <span className="text-xs font-medium">{tab.label}</span>
                    </button>
                ))}
                
                <button onClick={onAddClick} className="relative z-10 p-4 bg-purple-600 text-white rounded-full shadow-lg -translate-y-3.5 hover:bg-purple-700 transition-transform hover:scale-110">
                    <PlusCircleIcon className="w-8 h-8"/>
                </button>
                
                 <button onClick={() => setActiveTab('settings')} className={`relative flex flex-col items-center justify-center gap-1 p-2 rounded-full transition-colors w-16 h-16 z-10 ${activeTab === 'settings' ? 'text-purple-600 dark:text-purple-400' : 'text-zinc-500 hover:text-zinc-800 dark:hover:text-white'}`}>
...
({name, value, type="text", onChange, customOnChange}) => {
    ...
    return (
        <input
            ...
            className="w-full p-2 border rounded-md bg-white dark:bg-zinc-800 border-zinc-300 dark:border-zinc-600 focus:ring-2 focus:ring-purple-500 focus:outline-none"
        />
    );
};
...
const TagButton: React.FC<{active: boolean, children: ReactNode, onClick: () => void;}> = ({active, children, onClick}) => (
    <button type="button" onClick={onClick} className={`px-3 py-1 text-sm rounded-full border transition-colors ${active ? 'bg-purple-600 text-white border-purple-600' : 'bg-white dark:bg-zinc-700 border-zinc-300 dark:border-zinc-600 hover:border-purple-500'}`}>
